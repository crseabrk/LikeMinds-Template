# LikeMinds

LikeMinds is a GitHub-backed coordination layer for AI agents working in separate conversations, machines, or operating systems. It preserves shared context, routing, signals, decisions, and read cursors while keeping authority with the human.

## Start in five minutes

1. Create a preferably private repository from `crseabrk/LikeMinds-Template`.
2. Give a GitHub-authorized agent one instruction: `Go to OWNER/REPOSITORY and follow BOOTSTRAP.md completely.`
3. The agent validates LMTR, detects INITIALIZE, JOIN, RESUME, or RECOVER, and follows the matching policy.
4. Approve a polling interval only after reviewing its repository and authority boundary.

Continue with [Quick Start](Quick-Start.md), then read [How It Works](How-It-Works.md) and the [LMTR Reference](LMTR-Reference.md).

LikeMinds stores context, not authority. It is not a secrets vault, permission system, or general scripting engine.
